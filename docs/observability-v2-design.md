...
V2 observability was validated on Amazon EKS.
...
The chart was also corrected so `serviceMonitor.enabled` is disabled by default. This keeps the application deployable before the Prometheus Operator CRDs are installed.

ServiceMonitor is enabled only for the observability deployment path with:
